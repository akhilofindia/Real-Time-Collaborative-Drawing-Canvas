// client/canvas.js
// Handles all drawing, tools, rendering, FPS + latency, and user panels
// Exports initCanvas({ ws, userId, username, userColor, roomId })

export function initCanvas({ ws, userId, username, userColor, roomId }) {
  ...
  Object.assign(userPanel.style, {
    ...
    borderRadius: "10px", boxShadow: "0 2px 6px rgba(0,0,0,0.4)",
    fontFamily: "sans-serif", fontSize: "13px", zIndex: 1200,
    minWidth: "160px", maxWidth: "240px", userSelect: "none", lineHeight: "1.4em",
  });
    ...
    users.forEach((u) => {
      ...
      row.innerHTML = `
        <span style="display:inline-block;width:12px;height:12px;border-radius:50%;
                     background:${u.color};margin-right:8px;box-shadow:0 0 3px #000;"></span>
        ${u.userId === userId ? `<strong style="color:#00ffcc">${u.name} (You)</strong>` : `<span style="color:#ddd">${u.name}</span>`}
      `;
  ...
  function resizeCanvas() {
    ...
    // Save and restore image (best-effort)
    let saved;
    try { saved = ctx.getImageData(0, 0, canvas.width || 1, canvas.height || 1); } catch {}
    canvas.width = Math.max(1, w);
    canvas.height = Math.max(1, h);
    try { if (saved) ctx.putImageData(saved, 0, 0); } catch {}
    renderAll();
    renderCursors();
  }
  ...
  function renderCursors() {
    ...
    Object.entries(cursors).forEach(([id, c]) => {
      ...
      Object.assign(dot.style, {
        ...
        background: c.color, boxShadow: "0 0 2px rgba(0,0,0,0.3)"
      });
  ...
  // websocket helper (safeSend)
  ws.safeSend = ws.safeSend || function (o) { try { ws.send(JSON.stringify(o)); } catch (e) { } };
  ...
  function movePointer(e) {
    ...
    ws.safeSend({ type: "cursor", userId, x: pos.x, y: pos.y, color: userColor, name: username, roomId });
    ...
    if (currentTool === "brush" || currentTool === "eraser") {
      ...
      ws.safeSend({ type: "draw-segment", from: last, to: pos, color, width: strokeWidth, eraser: currentTool === "eraser", roomId });
      pointsBuffer.push(pos);
    } else if (currentTool === "rect" || currentTool === "circle") {
      previewShape = { type: "stroke", kind: "shape", shapeType: currentTool, from: startPoint, to: pos, color, width: strokeWidth };
      renderAll();
      ws.safeSend({ type: "shape-preview", shape: previewShape, roomId });
    }
  }
  ...
  function endPointer(e) {
    ...
    if (currentTool === "brush" || currentTool === "eraser") {
      ...
      previewShape = null;
      renderAll();
      ws.safeSend({ ...shapeStroke, roomId });
  ...
  if (clearBtn) clearBtn.addEventListener("click", () => ws.safeSend({ type: "clear", roomId }));
  if (undoBtn) undoBtn.addEventListener("click", () => ws.safeSend({ type: "undo", roomId }));
  if (redoBtn) redoBtn.addEventListener("click", () => ws.safeSend({ type: "redo", roomId }));

  // cleanup on page close
  window.addEventListener("beforeunload", () => {
    try { ws.safeSend({ type: "disconnect", userId, roomId }); } catch {}
  });

  // expose small API for debug
  window.__scribble = { userId, userColor, roomId, getHistory: () => localHistory.slice() };
  ...
  Object.assign(perfBox.style, {
    ...
    background: "rgba(0,0,0,0.75)", color: "#00ffcc", fontFamily: "monospace",
    fontSize: "13px", padding: "6px 10px", borderRadius: "8px",
    boxShadow: "0 2px 6px rgba(0,0,0,0.4)", zIndex: 1500, lineHeight: "1.4em",
    userSelect: "none"
  });
  ...
  setInterval(() => {
    ...
    ws.safeSend({ type: "ping", sentAt: t });
  }, 3000);

  // handle pong to compute latency
  ws.addEventListener("message", (ev) => {
    let m;
    try { m = JSON.parse(ev.data); } catch { return; }
    if (m.type === "pong") {
      latency = performance.now() - (m.sentAt || performance.now());
      updatePerfBox();
    }
  });
